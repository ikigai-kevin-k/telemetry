...
# Push CPU temperature to Prometheus Pushgateway for ARO-001-2
...
function push_metric() {
  ...
  echo "# HELP ${METRIC_NAME} Current system temperature in Celsius" > /tmp/metrics-aro001-2.txt
  echo "# TYPE ${METRIC_NAME} gauge" >> /tmp/metrics-aro001-2.txt
  echo "${METRIC_NAME}{instance=\"${INSTANCE_LABEL}\"} ${temperature_value}" >> /tmp/metrics-aro001-2.txt

  curl -sf \
    -X POST \
    --data-binary @/tmp/metrics-aro001-2.txt \
