...
setup_cron_jobs() {
    ...
    local cron_entries=(
        ...
        "0 2 * * * cd ${PROJECT_DIR} && ./scripts/maintenance/backup_telemetry_data.sh >> /var/log/telemetry_backup.log 2>&1"
        ""
        "# Health check every 6 hours"
        "0 */6 * * * cd ${PROJECT_DIR} && ./scripts/maintenance/check_and_restore_containers.sh --check-only >> /var/log/telemetry_health.log 2>&1"
        ""
...
ExecStart=${PROJECT_DIR}/scripts/maintenance/backup_telemetry_data.sh
...
User=ella
WorkingDirectory=${PROJECT_DIR}
ExecStart=${PROJECT_DIR}/scripts/maintenance/check_and_restore_containers.sh --check-only
StandardOutput=journal
StandardError=journal
...
#### Check System Health
```bash
./scripts/maintenance/check_and_restore_containers.sh
```
...
4. Run health check: `./scripts/maintenance/check_and_restore_containers.sh --check-only`
EOF
...
main() {
    ...
    if [ ! -f "scripts/maintenance/backup_telemetry_data.sh" ] || [ ! -f "scripts/maintenance/check_and_restore_containers.sh" ]; then
        print_error "Required scripts not found. Please ensure scripts/maintenance/backup_telemetry_data.sh and scripts/maintenance/check_and_restore_containers.sh exist."
        exit 1
    fi
    ...
    print_status "2. Test backup system: ./scripts/maintenance/backup_telemetry_data.sh"
    print_status "3. Test health check: ./scripts/maintenance/check_and_restore_containers.sh --check-only"
