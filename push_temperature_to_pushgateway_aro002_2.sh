...
# Push CPU temperature to Prometheus Pushgateway for ARO-002-2
...
INSTANCE_LABEL=${INSTANCE_LABEL:-"GC-ARO-002-2-agent"}
...
function push_metric() {
  ...
  echo "# HELP ${METRIC_NAME} Current system temperature in Celsius" > /tmp/metrics-aro002-2.txt
  echo "# TYPE ${METRIC_NAME} gauge" >> /tmp/metrics-aro002-2.txt
  echo "${METRIC_NAME}{instance=\"${INSTANCE_LABEL}\"} ${temperature_value}" >> /tmp/metrics-aro002-2.txt

  curl -sf \
    -X POST \
    --data-binary @/tmp/metrics-aro002-2.txt \
    "${PUSHGATEWAY_URL}/metrics/job/${JOB_NAME}/instance/${INSTANCE_LABEL}" >/dev/null
}
