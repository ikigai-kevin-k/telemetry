...
| zcam-tpe | 192.168.20.8 | tpe-001-1 | 192.168.20.10 | r008_tpe |
...
### 3. **文件**
- [ZCAM API Reference](ZCAM_API_Reference.md) - API 參考指南
- [ZCAM Multi Device Setup](ZCAM_Multi_Device_Setup.md) - 本設定指南
...
```bash
# 查詢特定設備的 RTMP 狀態
./zabbix/zcam_single_check.sh zcam-aro11 rtmp_status
# 輸出: busy

# 查詢電池電量
./zabbix/zcam_single_check.sh zcam-aro11 battery_level  
# 輸出: 100

# 查詢頻寬使用
./zabbix/zcam_single_check.sh zcam-aro11 bandwidth
# 輸出: 5.82678
```
...
### **ZCAM TPE (192.168.20.8) - New TPE Server**
- ✅ RTMP 狀態: busy (正在串流)
- ✅ 電池電量: 100%
- ✅ 攝影機模式: rec (錄影)
- 📊 頻寬: 5.81 Mbps
...
### **特別注意**
1. **aro22**: 頻寬為 0 但狀態為 busy，需要檢查
2. **aro22**: 自動重啟功能停用，建議啟用
